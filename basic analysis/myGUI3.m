...
function [hFig, hAxes] = createFigureAndAxes()
        ...
        hAxes.axis2 = createPanelAxisTitle(hFig,[0.55 0.50 0.4 0.4],'Magnitude Spectrum');
        hAxes.axis3 = createPanelAxisTitle(hFig,[0.1 0.05 0.4 0.4],'Peak Spectrum');
        hAxes.axis4 = createPanelAxisTitle(hFig,[0.55 0.05 0.4 0.4],'Frequency Spectrum');
end
    ...
    function insertButtons(hFig,hAxes,vid,vidDevice,fullPath)
         ...
         popup = uicontrol('Style', 'popup',...
           'String', {'VisionToolBox','OpticalFloor','ImageSubtraction'},...
...
function playCallback(hObject,~,vid,hAxes,vidDevice,fullPath)
    ...
    if val== 1
       % textingFunctionCall( vid,hAxes,vidDevice );
        visionToolBox( vid,hAxes,vidDevice,str2double(thresholdValue));
    elseif val==2
        textingFunctionCall( vid,hAxes,vidDevice,str2double(thresholdValue) );
    else
        imageSubstarction( vid,hAxes,vidDevice,str2double(thresholdValue) );
    end 
